...
from keras.layers import Embedding, add, concatenate, Multiply
...
def ecg_normal(shuju):
    a4 = []
    for i in range (len(shuju)):
        a5 = shuju[i,:]
        a6 = a5.reshape(-1,1)
        a7 = (a6 - a6.mean())/a6.std()
        a8 = a7.flatten()
        a4.append(a8)
    return a4
...
"""加载模型"""
model = load_model('D:/fangzhen/ECG/challenge/icbeb/model/two-atten.h5')
model.load_weights('D:/fangzhen/ECG/challenge/icbeb/model/two-weight-atten.h5')
...
def baseline(sample_path):
    ...
    test_ecg1 = ecg_normal(test_ecg1)
    test_ecg2 = ecg_normal(test_ecg2)
    test_ecg1 = np.array(test_ecg1)
    test_ecg2 = np.array(test_ecg2)   
    test_ecg3 = np.expand_dims(test_ecg1, axis=2)  
    test_ecg4 = np.expand_dims(test_ecg2, axis=2) 
    """预测"""   
    y_pre = model.predict([test_ecg3,test_ecg4])
    y_pre1 = y_pre.tolist()
...
if __name__ == '__main__':
    ...
    #测试数据文件夹
    RESULT_PATH = r'D:\fangzhen\ECG\challenge\icbeb\test_result\3'
    #测试结果文件夹
